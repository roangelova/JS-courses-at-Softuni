...
<article>
        ...
        <div id="result">${countMatched(towns, match)}</div>
    </article>
`;
...
function countMatched(towns, match){
   const matches = towns.filter(t => match && t.toLocaleLowerCase().includes(match.toLocaleLowerCase())).length;
   return matches ? `${matches} matches found` : 'No matches found';
}
